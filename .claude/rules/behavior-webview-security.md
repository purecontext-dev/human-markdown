...
- `script-src` uses nonces and `webview.cspSource` — no `eval`, no inline scripts, no `javascript:` URIs. The `cspSource` allowlists the extension's own origin for code-split chunks loaded via dynamic `import()`.
